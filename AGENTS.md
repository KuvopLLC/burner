...
| - | `am log [--item P] [--verb V] [-n N] [--json]` (semantic audit trail of state changes) |
| - | `am bug "TITLE"` (create a bug at top of priority, no estimate) |
| - | `am chore "TITLE"` (create a chore at top of priority, no estimate) |
| - | `am brief` (one-shot onboarding: project vision + dashboard + next pull + WIP + agreements) |
| `set_mode` | `am mode [coached\|autonomous]` (show or set the workflow mode) |
| `journal` | `am journal` (narrative audit trail grouped by story — the autonomous-mode review surface) |
| - | `am accept ITEM --note "..."` (record rationale; `--note` works on start/finish/deliver too) |

Convenience verb: `am pull ITEM` is `next` then `start` in one shot. The retro skill uses `am retro` for the summary surface.

### The audit trail (`am log`)

Every state-changing operation — `created`, `started`, `finished`, `delivered`, `accepted`, `rejected`, `set-type`, `estimate`, `bug`, `chore` — appends one line to `.am/activity.log` (actor, verb, item, detail, timestamp), through both the CLI and the MCP server. This is the "what have I been doing" feed: distinct from `am history ITEM` (which is git log for one file), `am log` is pre-commit, multi-item, and records intent. When working unattended, it is the trail a human reads to see what the agent did. Bugs and chores are born compliant: `am bug`/`am chore` set the type, strip the estimate, and drop the story at the top of priority so it is the next thing pulled — the "no fix without a bug, and the bug goes to the top" rule in one verb.
...
## Workflow modes: coached vs autonomous

The project runs in one of two modes, set in `.am/config.yaml` (`mode:`) and overridable per-session with the `AM_MODE` env var. Check it with `am mode`; the active mode also shows in `am brief` / `am journal`.

- **coached** (default) — the Pivotal stance. The dev pair drives `unstarted → delivered` and **stops**; the human (PM) accepts. The coach gate refuses a dev-pair `accepted`. Use this when a human is reviewing each story in real time. The pause is what turns "did it run" into "is this what the user needed".
- **autonomous** — for unattended runs. The seam is collapsed on purpose: you run the full loop, *including* `accepted`. In exchange, AM's value shifts from gating each story to **capturing intent and outcome**: record why each transition met the bar with `am accept ITEM --note "..."` (and `am finish`/`am deliver` accept `--note` too). The human reviews the record afterward with `am journal` — a per-story narrative of what was done and why — instead of gating live. A self-accept with no note is still logged honestly as `self-accepted (autonomous mode)`.

Switch with `am mode autonomous` / `am mode coached` (or set `AM_MODE` for one session). When you accept your own work in autonomous mode, always leave a `--note`: the journal is the only review surface the human has.

### Solo mode

When the same human is both the dev's pair and the PM and stays in **coached** mode, the seam still matters: stage the work, name the moment, and wait for the PM's answer instead of self-accepting. Reach for **autonomous** mode only when the human has explicitly stepped out of the loop and will review the journal after.
